...
void VillageManager::proc_move_human_to_location(
        int& human_id,
        int location_id,
        int value_for_erase
    ) {
    assert(human_id >= 0);
    this->at_village_register[location_id].push_back(human_id);
    this->v_hmn_mgr->set_human_at_village(human_id, location_id);
    this->human_already_moved_in_this_step[human_id] = true;
    
    human_id = value_for_erase;
}
void VillageManager::proc_return_human_to_home(
        int& human_id,
        int value_for_erase
    ) {
    this->proc_move_human_to_location(
        human_id,
        this->v_hmn_mgr->get_home_village(human_id),
        value_for_erase
    );
}
float VillageManager::func_dist_to_weight(float dist) {
    if (dist > 0) {
...
void VillageManager::build_static_mobility_network(){
    ...
    for (int vv1 = 0; vv1 < this->sum_num_villages; vv1++){
        if (this->location_type_of[vv1] != common::LocationType::kVillage) {
            continue;
        }
         ...
         for (int vv2 = 0; vv2 < this->sum_num_villages; vv2++){
            ...
            // longit2=G_villages[i]->longitude;
                  ...
                  if (vv1 == vv2){
                    // prob[d][i]=0;
                    prob_vv1[vv2] = 0;
                  } else if (this->location_type_of[vv2] != common::LocationType::kVillage) {
                    prob_vv1[vv2] = 0;
                  }
                  else{
         ...
         for (int vv3 = 0; vv3 < this->sum_num_villages; vv3++){
              ...
              for (int q=0;q < nprob_vv1[vv3];q++){
                  // village->staticnet.push_back(w);
                    assert(this->location_type_of[vv3] == common::LocationType::kVillage);
...
void VillageManager::step_mobility(int time_step) {
    if (this->kMobility_enabled || this->kMobility_forest_enabled) {
        
        bool forest_called = false;

        if (this->kMobility_forest_enabled) {
            this->forest_banned = (time_step >= this->kForest_ban_from
                                    && time_step - this->kForest_ban_from < this->kForest_ban_length );
            this->forest_entry_turn_back = time_step == this->kForest_ban_from;
            // force turn back only on the first day of the ban
        }

        int num_moves_forest = 0;
        int num_moves_static = 0;

        std::fill(
            this->human_already_moved_in_this_step.begin(),
            this->human_already_moved_in_this_step.end(),
            false
        );

        if (this->kMobility_forest_enabled) {
            if (util::get_rand_uniform() < this->kForest_precedence_over_static_mobility) {
                num_moves_forest = this->step_population_movement_forest();
                std::cout << "\tnum_forest_movements=" << num_moves_forest << "\n";
                forest_called = true;
            }
        }

        if (this->kMobility_enabled) {
            num_moves_static = this->step_population_movement_static();
            std::cout << "\tnum_static_movements=" << num_moves_static << "\n";
        }

        if (this->kMobility_forest_enabled && !forest_called) {
            num_moves_forest = this->step_population_movement_forest();
            std::cout << "\tnum_forest_movements=" << num_moves_forest << "\n";
            forest_called = true;
        }

        this->log_daily_num_moves_static = num_moves_static;
        this->log_daily_num_moves_forest = num_moves_forest;
...
int VillageManager::step_population_movement_static(
    ...
    if (this->kMobility_enabled) {
        ...
        return this->step_population_movement_static_kernel(
            ...
            this->v_hmn_mgr->get_mobility_of(),
            this->v_hmn_mgr->get_home_village_of(),
...
int VillageManager::step_population_movement_static_kernel(
...
// static std::chrono::duration<double, std::milli> time_erase;
    ...
    for (int vv = 0; vv < village_array_size; vv++) {
        if (this->location_type_of[vv] != common::LocationType::kVillage) {
            continue;
        }
        ...
        for (auto& hh : villager_reg[vv]) {
            ...
            rndm_nmbr_if_move = util::get_rand_uniform();
            ...
            if (human_home_village_array[hh] == vv) {
                ...
                if (rndm_nmbr_if_move < kMove_out_rate) {
                    ...
                    rndm_nmbr_move_to = util::get_rand_uniform();
    ...
    // time_target_mid += t_mid_target - t_start_target;
                    ...
                    assert(this->location_type_of[target_village] == common::LocationType::kVillage);
