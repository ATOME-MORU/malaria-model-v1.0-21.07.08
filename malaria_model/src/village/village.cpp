...
VillageManager::VillageManager(
        ...
        kMalaria_post_rate_reduction_enabled(malaria_post_rate_reduction_enabled),
        kMalaria_post_rate_reduction_size(malaria_post_rate_reduction_size),
        kMalaria_post_rate_reduction_start_on_day(malaria_post_rate_reduction_start_on_day),
        kMalaria_post_rate_reduction_slope_days(malaria_post_rate_reduction_slope_days),
        kMalaria_post_rate_reduction_length_days(malaria_post_rate_reduction_length_days),

        kTreatment_rate_mda(treatment_rate_mda),
        kMda_max_age(mda_max_age),
        kMda_min_age(mda_min_age),
        ...
        kTreatment_coverage_tact(treatment_coverage_tact),
        kTreatment_drug_first_line_a(static_cast<common::DrugName>(treatment_drug_first_line_a)),
        kTreatment_drug_first_line_b(static_cast<common::DrugName>(treatment_drug_first_line_b)),
        kTreatment_tact_switch_day(treatment_tact_switch_day),
        kTreatment_tact_first_line_increase(tact_first_line_increase),
        kTreatment_tact_first_line_max(treatment_tact_first_line_max),

        kTreatment_tact_a7_enabled(treatment_tact_a7_enabled),
        kTreatment_daily_inc_a7(treatment_daily_inc_a7),

        kTreatment_tact_a11_enabled(treatment_tact_a11_enabled),
        kTreatment_tact_a11_init_tact_rate(treatment_tact_a11_init_tact_rate),
        kTreatment_tact_a11_num_years(treatment_tact_a11_num_years),
        kTreatment_tact_a11_daily_inc(
            (1.0 - treatment_tact_a11_init_tact_rate) / 
            (treatment_tact_a11_num_years * static_cast<float>(common::kNum_days_in_one_year))
        ),

        kTreatment_tact_use_private_drugs(treatment_tact_use_private_drugs),

        kTreatment_coverage_forest(treatment_coverage_forest),
        ...
        kMosquito_seasonality_init_file_horizontal_shift_days(mosquito_seasonality_init_file_horizontal_shift_days),
        kSeasonality_base_percent(seasonality_base_percent),
        ...
        kMobility_forest_enabled(mobility_forest_enabled),
        kForest_precedence_over_static_mobility(forest_precedence_over_static_mobility),
        kForest_set_off_probability(forest_set_off_probability),
        kForest_return_home_probability(forest_return_home_probability),
        kStay_at_entry_points(stay_at_entry_points),
        kReturn_stay_at_entry_points(return_stay_at_entry_points),
        kFunc_mobility_forest_version(func_mobility_forest_version),
        kReturn_early(return_early),
        kMin_num_way_points(min_num_way_points),
        kFollow_routes(follow_routes),
        kForest_ban_from(ban_from),
        kForest_ban_length(ban_length),
        kForest_ban_compliance(ban_compliance),

        kAtt_use_sampler_list(att_use_sampler_list),
        kAtt_sampler_list_unit(att_sampler_list_unit),
    ...
    if (!input_files_stream) {
        ...
        forest_goer_percentage_of = new float[rows_to_read];
        location_type_of = new common::LocationType[rows_to_read];
        mosquito_spawning_site_size_of = new float[rows_to_read];
        ...
        for (int vv = 0; vv < rows_to_read; vv++){
            ...
            size_t num_parasite_types = common::kParasiteType_size;
        ...
        for (int i = 0; i < rows_to_read; i++) {
            ...
            // default values for village property
            this->id_of[i] = -1;
            this->population_of[i] = 0;
            this->longitude_of[i] = 0.0;
            this->latitude_of[i] = 0.0;
            this->transmission_coefficient_of[i] = 0.0;
            this->num_malaria_post_of[i] = 0;
            this->num_days_malaria_post_opened_of[i] = 0;
            this->migrant_population_percentage_of[i] = 0.0;

            this->forest_goer_percentage_of[i] = 0.0;
            this->location_type_of[i] = common::LocationType::kVillage;
            this->mosquito_spawning_site_size_of[i] = 0.0;

            this->infection_rates_of[i][static_cast<int>(common::ParasiteType::kR0)] = 0.0;
            this->infection_rates_of[i][static_cast<int>(common::ParasiteType::kRa)] = 0.0;
                ...
                while (std::getline(ss, item, input_file_delimiter)) {
                    ...
                    if (j <= num_columns) {
                        ...
                        switch (j) {
                            ...
                            case columns::kForest_goer_percentage:
                                this->forest_goer_percentage_of[i] = std::stof(item);
                                break;

                            case columns::kLocation_type:
                                if (common::kValid_location_type_chars.find(item[0]) != std::string::npos) {
                                    this->location_type_of[i] = static_cast<common::LocationType>(item[0]);
                                } else {
                                    std::cout << "\033[31;47mVillageManager: Unexpected charactor for location type."
                                              << " (Expecting a charactor from \""
                                              << common::kValid_location_type_chars << "\" got \""
                                              << item[0] << "\" at row " << i << "), defaulting to \"v\""
                                              << "\033[0m" << std::endl;
                            ...
                            case columns::kMosquito_spawning_site_size:
                                float mss_size = std::stof(item);
                                if (mss_size < 0.0) {
                                    std::cout << "\033[31;47mVillageManager: Invalid value ("
                                              << mss_size << ") for spawning site size at"
                                              << " at row " << i << "), defaulting to 0.0"
                                              << "\033[0m" << std::endl;
                                    if (!this->kIf_batch_mode) {
                                        std::cin.ignore();
                                    }
                                ...
                                this->mosquito_spawning_site_size_of[i] = mss_size;
                                break;
        ...
        if (this->kMosquito_seasonality_init_if_from_file) {
            util::read_vector_from_file(this->mosquito_seasonality_curve, this->kMosquito_seasonality_init_file_name);
        }
        ...
        // Mobility
        this->human_already_moved_in_this_step.resize(this->sum_total_population);
        this->last_entry_point_visited.resize(this->sum_total_population);
...
VillageManager::~VillageManager() {
    ...
    if (forest_goer_percentage_of) {delete[] forest_goer_percentage_of;}
    if (location_type_of) {delete[] location_type_of;}
    if (mosquito_spawning_site_size_of) {delete[] mosquito_spawning_site_size_of;}
...
void VillageManager::update_summary(
    ...
    for (int vv = 0; vv < this->sum_num_villages; vv++) {
        ...
        for (size_t pp = 0; pp < common::kParasiteType_size; pp++) {
    ...
    this->daily_record_of_mosquito_infected_sum += pit_num_infected;
    this->daily_record_of_mosquito_infectious_sum += pit_num_infectious;
...
void VillageManager::print_one(const int index_village) const{
        std::cout   << std::setw(5) << id_of[index_village]
                    << std::setw(5) << location_type_of[index_village]
                    << ", pop:" << std::setw(3) << population_of[index_village]
                    ...
                    << ", f-g:" << std::setprecision(3) << std::setw(4) << forest_goer_percentage_of[index_village] * 100 << "%"
...
void VillageManager::treatment_by_malaria_post(
    ...
    float treatment_rate_reduction_size = 0.0;
    if ( this->kMalaria_post_rate_reduction_enabled ) {

        if ( time_step > kMalaria_post_rate_reduction_start_on_day) {

            if ( time_step <= kMalaria_post_rate_reduction_start_on_day + kMalaria_post_rate_reduction_length_days ) {

                if ( (time_step - kMalaria_post_rate_reduction_start_on_day) >= kMalaria_post_rate_reduction_slope_days ) {

                    treatment_rate_reduction_size = kMalaria_post_rate_reduction_size;

                } else {

                    assert(kMalaria_post_rate_reduction_slope_days != 0);

                    // down slope
                    treatment_rate_reduction_size = kMalaria_post_rate_reduction_size
                                        * static_cast<float>(time_step - kMalaria_post_rate_reduction_start_on_day)
                                        / static_cast<float>(kMalaria_post_rate_reduction_slope_days);

                }

            } else if (time_step <= kMalaria_post_rate_reduction_start_on_day + kMalaria_post_rate_reduction_length_days + kMalaria_post_rate_reduction_slope_days) {

                // up slope
                treatment_rate_reduction_size = kMalaria_post_rate_reduction_size - kMalaria_post_rate_reduction_size
                                        * static_cast<float>(time_step - kMalaria_post_rate_reduction_start_on_day - kMalaria_post_rate_reduction_length_days)
                                        / static_cast<float>(kMalaria_post_rate_reduction_slope_days);
    ...
    assert(treatment_rate_reduction_size<=kMalaria_post_rate_reduction_size);
    assert(treatment_rate_reduction_size>=0.0);
        ...
        for (const auto& hhi : at_vll_reg.at(vv)) {
            ...
            if (existing_drug[hhi] != common::DrugName::kPiperaquine
                ...
                if (dominant_type[hhi] != common::ParasiteType::kNoParasite) {
                    ...
                    // if (treatment_rate_reduction_size > 0) {
                    //     std::cout << "rate:" << applicable_rate << std::endl;
                    //     std::cout << "reduced by:" << treatment_rate_reduction_size << std::endl;
                    // }

                    if (rnd_nmbrs[hhi] < applicable_rate * (1-treatment_rate_reduction_size)) {
                        given_drug[hhi] = kMalaria_post_drug;
                        mp_treatment_counter++;
                        daily_record_of_new_treatments_by_parasite_type[static_cast<int>(dominant_type[hhi])]++;
                    }
                }
...
void VillageManager::treatment_by_mda_kernel(
    ...
    for (const auto& hhi : list_of_humans) {
        ...
        if (hmn_age_of[hhi] <= mda_max_age && hmn_age_of[hhi] >=mda_min_age) {

            // std::cout << "mda on age " << hmn_age_of[hhi] << std::endl;
...
void VillageManager::step_treatment_general(
    ...
    } else if ( this->kFunc_treatment_general_version == "tact") {

        this->func_general_treatment_tact(
            hmn_mgr->get_given_drug(),
            bld_mgr->get_drug_of(),
    ...
    } else if ( this->kFunc_treatment_general_version == "forest") {

        this->func_general_treatment_forest(
            hmn_mgr->get_given_drug(),
            bld_mgr->get_drug_of(),
            bld_mgr->get_drug_given_of(),
            bld_mgr->get_drug_given_day_of(),
...
void VillageManager::func_general_treatment_forest(
        common::DrugName* given_drug,
    ...
    // (void) mellow_rate;
    const float kTreatment_rate_if_clinical = 1.0 - pow(1.0-this->kTreatment_coverage_forest, mellow_rate);
    ...
    const common::DrugName kDrug = common::DrugName::kPiparte;
    ...
    for (uint vv = 0; vv < at_vll_reg.size(); vv++) {
        ...
        if (this->get_location_type(vv) != common::LocationType::kVillage ) {
            continue;
        }
        ...
        for (const auto& hh : at_vll_reg.at(vv)) {
            if (is_clinical_of[hh]) {
                ...
                if (drug_of[hh] == common::DrugName::kNoDrug){
                    ...
                    if (util::get_rand_uniform() < kTreatment_rate_if_clinical) {
...
void VillageManager::func_general_treatment_tact(
        common::DrugName* given_drug,
        const common::DrugName* drug_of,
        const common::DrugName* drug_given_of,
        const int* drug_given_day_of,
    ...
    const float kTreatment_rate_if_clinical = 1.0 - pow(1.0-this->kTreatment_coverage_tact, mellow_rate);

    float kProb_first_line = 0.05;
    if (this->kTreatment_tact_first_line_increase) {
        // A7
        if (this->kTreatment_tact_a7_enabled
            && (time_step >= this->kTreatment_tact_switch_day)) {
                kProb_first_line += static_cast<float>(kTreatment_tact_switch_day - 1) * this->kTreatment_daily_inc_first_line;
                kProb_first_line += static_cast<float>(time_step - this->kTreatment_tact_switch_day + 1) * this->kTreatment_daily_inc_a7;
        } else {
            kProb_first_line += this->kTreatment_daily_inc_first_line * static_cast<float>(time_step);
        }
    }
    kProb_first_line = std::min(kProb_first_line, this->kTreatment_tact_first_line_max);

    if (!this->kTreatment_tact_use_private_drugs) {
        kProb_first_line = 1.0;
    }

    // A11
    float prob_first_line_b = 1.0;
    if (this->kTreatment_tact_a11_enabled && time_step >= this->kTreatment_tact_switch_day) {
        prob_first_line_b = this->kTreatment_tact_a11_init_tact_rate;
        prob_first_line_b += this->kTreatment_tact_a11_daily_inc *
                            static_cast<float>(1 + time_step - this->kTreatment_tact_switch_day);
    }

    for (size_t vv = 0; vv < at_vll_reg.size(); vv++) {
        for (const auto& hh : at_vll_reg[vv]) {
            if (is_clinical_of[hh]) {
                if (drug_of[hh] == common::DrugName::kNoDrug) {
                    if (util::get_rand_uniform() < kTreatment_rate_if_clinical) {
                        ...
                        if (drug_given_day_of[hh] >= 0) {
                            if ((time_step - drug_given_day_of[hh]) < drug_failure_if_clinical_on_day ) {
                                daily_record_of_drug_failure_by_new_treatment[
                                    static_cast<int>(drug_given_of[hh])
                                ]++;
                            }
                        }

                        common::DrugName drg = common::DrugName::kNoDrug;

                        if (util::get_rand_uniform() < kProb_first_line) {                        
                            if (time_step >= this->kTreatment_tact_switch_day) {
                                
                                if (this->kTreatment_tact_a11_enabled) {
                                    if (util::get_rand_uniform() < prob_first_line_b) {
                                        drg = this->kTreatment_drug_first_line_b;
                                    } else {
                                        drg = this->kTreatment_drug_first_line_a;
                                    }
                                } else {
                                    drg = this->kTreatment_drug_first_line_b;
                                }
                            ...
                            size_t sample_size = this->kTreatment_tact_prvt_drugs.size();
                            drg = this->kTreatment_tact_prvt_drugs[
                                    util::get_random_index_from_vector_of_size(sample_size)
                                  ];
                            // drg = common::DrugName::kAL;
                        }

                        given_drug[hh] = drg;

                        daily_record_of_new_treatments_by_parasite_type[
                            static_cast<int>(dominant_type_of[hh])
                        ]++;
                        assert(dominant_type_of[hh] != common::ParasiteType::kNoParasite);
                    }
                }
            }
        }
...
void VillageManager::init_get_villagers(human::HumanManager* human_manager){
    ...
    for (int vv = 0; vv < this->sum_num_villages; vv++) {
        ...
        this->at_village_register[vv].clear();
        ...
        for (int hh = 0; hh < this->population_of[vv]; hh++) {
            ...
            if (util::get_rand_uniform() < this->forest_goer_percentage_of[vv]) {
                human_manager->set_human_is_forest_goer(human_index, true);
            } else {
                human_manager->set_human_is_forest_goer(human_index, false);
            }

            human_index++;
        }
    }
    assert(human_index == human_manager->sum_num_humans);
...
void VillageManager::init_infections_kernel(
    ...
    for (int vv = 0; vv < vll_array_size; vv++) {
        ...
        // std::cout << "village_infected_population:" << village_infected_population << "\n";
        ...
        for (auto pt: util::Enum_iterator<common::ParasiteType>()) {
            ...
            if (pt == common::ParasiteType::kNoParasite) {
                ...
                for (int hh_of_village = 0; hh_of_village < per_pt_pop; hh_of_village++){
...
void VillageManager::reassign_parasitetypes_mosq(
            const std::vector<common::ParasiteType>& pt_sample_list
            ) {
    const size_t kSample_size = pt_sample_list.size();
    for (int vv = 0; vv < this->sum_num_villages; vv++) {
        int infected_sum = 0;
        int infectious_sum = 0;
        for (size_t pp = 0; pp < common::kParasiteType_size; pp++) {
            infected_sum += this->mosquito_infected_composition_of[vv][pp];
            infectious_sum += this->mosquito_infectious_composition_of[vv][pp];
            this->mosquito_infected_composition_of[vv][pp] = 0;
            this->mosquito_infectious_composition_of[vv][pp] = 0;
        }
        for (int ii = 0; ii < infected_sum; ii++) {
            int pt_index = static_cast<int>(pt_sample_list.at(
                    util::get_random_index_from_vector_of_size(kSample_size)
                ));
            this->mosquito_infected_composition_of[vv][pt_index]++;
        }
        for (int ii = 0; ii < infectious_sum; ii++) {
            int pt_index = static_cast<int>(pt_sample_list.at(
                    util::get_random_index_from_vector_of_size(kSample_size)
                ));
            this->mosquito_infected_composition_of[vv][pt_index]++;
...
void VillageManager::step_update_beta_season_of(int at_time_step){
    for (int vv = 0; vv < this->sum_num_villages; vv++) {
            ...
            if (this->kMosquito_seasonality_init_if_from_file) {
                ...
                // std::cout<<"seasonality="<<this->mosquito_seasonality_on[at_time_step + this->kSeasonality_step_offset]<<std::endl;
                // std::cin.ignore();
                int curve_vector_size = static_cast<int>(this->mosquito_seasonality_curve.size());

                int t = (at_time_step + this->kMosquito_seasonality_init_file_horizontal_shift_days) % curve_vector_size - 1;
                ...
                assert(t >= 0);
                assert(t < curve_vector_size);

                this->beta_season_of[vv] =
                    this->func_beta_to_beta_season_given_curve(
                        beta,
                        this->mosquito_seasonality_curve[t],
                        this->kSeasonality_base_percent,
                        this->kSeasonality_amplitude
                    );
...
float VillageManager::func_beta_to_beta_season_given_curve(
        float beta,
    ...
    return seasonality_base_percent * beta
            + ( 1.0 - seasonality_base_percent ) * beta * seasonality_multiplier * seasonality_amplitude;
}
...
void VillageManager::step_update_at_village_attractiveness() {
    if (this->v_hmn_mgr->get_if_attractiveness_enabled()) {
        if (this->kMobility_enabled
            || this->kMobility_forest_enabled
            || (this->kAtt_use_sampler_list && this->at_village_att_sampler.empty())
            || (!this->kAtt_use_sampler_list && this->at_village_attractiveness.empty()) ) {
            
            if (this->kAtt_use_sampler_list) {
                std::cout << "building sampler..." << std::endl;
                std::vector<std::vector<int>> new_at_village_att_sampler;
                new_at_village_att_sampler.resize(this->sum_num_villages);
                for (int vv = 0; vv < this->sum_num_villages; vv++) {
                    for (const int & hh : this->at_village_register[vv]) {
                        for (int jj = 0;
                                jj < this->v_hmn_mgr->get_attractiveness(hh)*this->kAtt_sampler_list_unit;
                                jj++) {
                            new_at_village_att_sampler[vv].push_back(hh);
                        }
                    }
                }
                this->at_village_att_sampler = new_at_village_att_sampler;
                // std::cout << "at_village_att_sampler[0].size() = " << this->at_village_att_sampler[0].size() << "\n";
                // std::cin.ignore();
            } else {
                ...
                // std::cout << "at_village_attractiveness.size() = " << this->at_village_attractiveness.size() << "\n";
                // std::cin.ignore(); 
            }
...
std::vector<int> VillageManager::func_get_humans_to_bite(int num_new_bites, int village_id) {
    ...
    if (this->v_hmn_mgr->get_if_attractiveness_enabled()) {
        ...
        if (!this->at_village_attractiveness_updated) {
            this->step_update_at_village_attractiveness();
        }

        if (this->kAtt_use_sampler_list) {
            size_t sample_size = this->at_village_att_sampler[village_id].size();
            for (int bb = 0; bb < num_new_bites; bb++) {
                humans_to_bite[bb] = this->at_village_att_sampler[village_id][
                                        util::get_random_index_from_vector_of_size(sample_size)
                                      ];
            }
        } else {
        ...
        for (const int& human_index: humans_to_bite) {
            ...
            if ( util::get_rand_uniform() < bite_success_threshold ) {
                ...
                if (using_ode) {
                    ...
                    infections_array[human_index] = static_cast<common::ParasiteType>(bite_parasite_index);
            ...
            // if (parasite_types.at(pp) != common::ParasiteType::kNoParasite) {
                ...
                for (int bb = 0; bb < infectious_msq_composition_array[vv][pp]; bb++) {
                    ...
                    if ( rnd_nmbr < bite_success_threshold ) {
                        ...
                        infections_array[human_index] = static_cast<common::ParasiteType>(pp);
...
void VillageManager::infection_m2h_kernel_per_bite(
    ...
    for(int vv = 0; vv < vll_array_size; vv++){
        ...
        // 1. # infectious mosquito to # infectious bites
                ...
                // TODO: continue if no mosq of this type
                ...
                // std::cout << num_new_bites << ", " << vv << "\n";

                if (num_new_bites == 0) {
                    continue;
                }

                if (this->get_current_population_of_village(vv) == 0) {
                    daily_record_of_new_bites[vv] += num_new_bites;
                    daily_record_of_new_bites_sum += num_new_bites;
                    daily_record_of_wasted_bites[vv] += num_new_bites;
                    daily_record_of_wasted_bites_sum += num_new_bites;
                    continue;
                }
                ...
                // std::cout << "1.1 good here\n";
                // std::cout << human_index << "\n";
                        daily_record_of_new_bites[vv]++;
                        daily_record_of_new_bites_sum++;
                // std::cout << "1.2 good here\n";
...
void VillageManager::mosquito_incubation_step_kernel(
    ...
    for (int vv = 0; vv < array_size_vv; vv++) {
        // for (int pp = 1; pp < array_size_pp; pp++) { // note that this start at 1, ignoring kNoParasite
        for (size_t pp = 0; pp < common::kParasiteType_size; pp++) {
            int temp_num_bites_to_incubate = infected_composition_array[vv][pp];
...
void VillageManager::get_mosquito_count_of_village(
        int village_id,
        int& infected,
        int& infectious
    ) const {
    ...
    // for (int pp = 1; pp < static_cast<int>(this->parasite_type_lookup.size()); pp++) {
    for (size_t pp = 0; pp < common::kParasiteType_size; pp++) {
        infected += this->mosquito_infected_composition_of[village_id][pp];
        infectious += this->mosquito_infectious_composition_of[village_id][pp];
    }
}
