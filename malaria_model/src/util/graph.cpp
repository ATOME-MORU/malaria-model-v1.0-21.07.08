...
#include <algorithm> // std::max_element, std::reverse
...
VillageGraph::VillageGraph(
    ...
    std::vector<int> ids_of_vll(num_vll);
    std::iota(std::begin(ids_of_vll), std::end(ids_of_vll), 0); // 0, 1, 2, ..., num_vll-1 
    this->init(distances, ids_of_vll);
...
VillageGraph::VillageGraph(
    const int num_vll,
    const float* const* distances,
    const std::vector<int>& ids_of_vll
): vg(num_vll){
    this->init(distances, ids_of_vll);
}

void VillageGraph::init(
        const float* const* distances,
        const std::vector<int>& ids_of_vll
    ) {

    assert(boost::num_vertices(this->vg) == ids_of_vll.size());
    
    this->num_villages = static_cast<int>(ids_of_vll.size());
    ...
    for (int vv=0; vv < this->num_villages; vv++) {
        vg[boost::vertex(vv,vg)].id = ids_of_vll.at(vv);
    }
...
std::vector<int> VillageGraph::find_shortest_paths_route(int from_vll_idx, int to_vll_idx) const {
    return this->find_shortest_paths_route(from_vll_idx, to_vll_idx, false);
}
// std::vector<VillageGraph::Graph::vertex_descriptor> VillageGraph::find_shortest_paths_route(int from_vll_idx, int to_vll_idx) const {
std::vector<int> VillageGraph::find_shortest_paths_route(int from_vll_idx, int to_vll_idx, bool verbose) const {
    std::vector<int> route;
    std::vector<int> route_degree;
    float dist = this->find_shortest_paths_route(
                        from_vll_idx,
                        to_vll_idx,
                        route,
                        route_degree,
...
float VillageGraph::find_shortest_paths_route(
        int from_vll_idx,
        int to_vll_idx,
        std::vector<int>& route,
        std::vector<int>& route_degree,
    ...
    assert(from_vll_idx >= 0);
    assert(to_vll_idx   >= 0);

    assert(from_vll_idx < static_cast<int>(boost::num_vertices(this->vg)));
    assert(to_vll_idx   < static_cast<int>(boost::num_vertices(this->vg)));

    // std::vector<int> route;
    // std::vector<int> route_degree;
    route.clear();
    route_degree.clear();
    // std::vector<Graph::vertex_descriptor> route;
    std::vector<Graph::vertex_descriptor> p(boost::num_vertices(this->vg));
    std::vector<float> d(boost::num_vertices(this->vg));

    boost::dijkstra_shortest_paths(this->vg, boost::vertex(from_vll_idx, this->vg),
        predecessor_map(boost::make_iterator_property_map(p.begin(), boost::get(boost::vertex_index, this->vg)))
        .distance_map(boost::make_iterator_property_map(d.begin(), boost::get(boost::vertex_index, this->vg)))
    );

    Graph::vertex_descriptor v_current, v_from;
    v_from = boost::vertex(from_vll_idx, this->vg);
    v_current = boost::vertex(to_vll_idx, this->vg);
    while (v_current != v_from) {
        // route.push_back(v_current);
        route.push_back(this->vg[v_current].id);
        route_degree.push_back(boost::out_degree(v_current, this->vg));
        v_current = p[v_current];
    }

    std::reverse(route.begin(), route.end());
    std::reverse(route_degree.begin(), route_degree.end());
    if (verbose) {
        std::cout << "route from " << from_vll_idx
                    << " to " << to_vll_idx 
                    << "(dist: " << d[to_vll_idx] << ")" << ": ";
        // for (auto vv : route) {
        for (size_t vv = 0; vv < route.size(); vv++) {
            std::cout << route[vv] << "(" << route_degree[vv] <<  "), ";
        }
        std::cout << "\n";
    }
...
float VillageGraph::get_edge_weight(int ii, int jj) const {
    std::pair<Edge, bool> ee = boost::edge(ii, jj, this->vg);
    if (ee.second) {
        return boost::get(boost::edge_weight_t(), this->vg, ee.first);
    }
    return 0.0;
}

int VillageGraph::get_degree_of_village(int ii) const {
    return boost::out_degree(boost::vertex(ii, this->vg), this->vg);
}
