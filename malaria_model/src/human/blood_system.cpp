...
BloodSystemManager::BloodSystemManager (
        ...
        float drug_b_effect_on_ra,
        float drug_ab_effect_on_ra,
        float drug_a_effect_on_rb,
        ...
        int mutation_start_at_time_step,
        ...
        float per_infection_fixation_base_rate,

        bool report_genotype_weights,
        ...
        kMutation_enabled(mutation_enabled),
        kMutation_start_at_time_step(mutation_start_at_time_step),
        ...
        kPer_infection_fixation_base_rate(per_infection_fixation_base_rate),
        kReport_genotype_weights(report_genotype_weights),
...
void BloodSystemManager::init_from_file(
        ...
        common::DrugManager* const dg_mgr
    ){

    // const int kNum_columns = 5;

    std::cout << "BloodSystemManager::init_from_file(" << input_file_name << ")" << std::endl;
    ...
    for (int ss = 0; ss < this->sum_num_systems; ss++) {
        if(!std::getline(input_file_stream, str_line).eof()) {
            ...
            for (int ii = (target_moi - static_cast<int>(this->moi_of[ss])); ii > 0; ii-- ) {
                new_pg_index = this->add_one_pg_with_defaults(
                    ss,
                    common::ParasiteType::kR0,
    ...
    this->parasite_dominance(dg_mgr);
...
void BloodSystemManager::parasite_injection(
    ...
    for (int ii = 0; ii < num_cases; ii++) {
        ...
        // if (injected_already[system_index] || this->tail_pg_of[system_index] < 0 ) {
        if (injected_already[system_index]
            || this->most_advanced_stage_of[system_index] != common::StageName::kInfectious
            || common::GenotypeManager::gt_has_mutations(
                static_cast<common::genotype_t>(this->dominant_type_of[system_index]),
                static_cast<common::genotype_t>(parasite_type_to_inject)
                )
            ) {
            ii--;
        } else {
...
int BloodSystemManager::find_rand_clean_system() const {
    const size_t kSample_size = this->sum_num_systems;
    size_t try_cout = 0;
    while (try_cout < kSample_size) {
        int ss = static_cast<int>(util::get_random_index_from_vector_of_size(kSample_size));
        if (!this->is_infected(ss)) {
            return ss;
        }
        try_cout++;
    }
    throw std::runtime_error("Exception: bm is unable to find a clean host.");
    return -1;
}

void BloodSystemManager::parasite_importation(
        int num_parasites_to_import,
        common::ParasiteType pt,
        int time_step
    ) {
    const common::StageName kStg_import = common::StageName::kInfectious;
    for (int ii = 0; ii < num_parasites_to_import; ii++) {
        int ss = this->find_rand_clean_system();
        this->add_one_pg_with_defaults(
            ss, pt, kStg_import,
            static_cast<uint16_t>(time_step)
        );
        this->moi_of[ss]++;
        this->dominant_type_of[ss] = pt;
        this->most_advanced_stage_of[ss] = kStg_import;
        assert(this->is_infected(ss));
    }

}

void BloodSystemManager::reassign_parasitetypes(
        const std::vector<common::ParasiteType>& pt_sample_list
    ) {
    size_t sample_size = pt_sample_list.size();
    for (int ss = 0; ss < this->sum_num_systems; ss++) {
        if (this->is_infected(ss)) {
            common::ParasiteType pt = pt_sample_list.at(
                    util::get_random_index_from_vector_of_size(sample_size)
                );
            for (int pp = this->head_pg_idx(ss); pp <= this->tail_pg_idx(ss); pp++) {
                this->set_pg_type(pp, pt);
            }
        }
    }
}
...
void BloodSystemManager::drug_intake(
        const common::DrugName* const drug_list,
        const int time_step
    ){
...
void BloodSystemManager::drug_loss(
    ...
    for (int ss = 0; ss < this->sum_num_systems; ss++) {
        if (dg_mgr->drug_loss(this->drug_of[ss])) {

            if (this->drug_of[ss] == common::DrugName::kNoDrug
                && this->tail_pg_of[ss] >= 0) {

                this->sum_tf_by_drug_loss_count[
                    static_cast<int>(this->dominant_type_of[ss])
                ] += 1;
            }
...
// A4, DHA-PIP
    ...
    // const float kDrug_loss_Piperaquine = 1.0/30.0; // G_xab, A4
    ...
    // const float kDrug_loss_Piparte_to_Piperaquine = 1.0/4.0; // G_xai // mmc_wp2_rev5, 5562e42, A4
    ...
    for (int ss = 0; ss < this->sum_num_systems; ss++) {
        switch (this->drug_of[ss]) {
            ...
            case common::DrugName::kNoDrug:
                break;
            default :
                std::cout << "drug_loss of " << this->drug_of[ss]
                            << " not defined.\n";
                break;
        }
        ...
        if (this->head_pg_of[ss] < 0){
            ...
            for (int pp = lbound + this->head_pg_of[ss]; pp <= lbound+this->tail_pg_of[ss]; pp++){
                ...
                if (kIf_probability_based) {
                    ...
                    if_cleared = dg_mgr->drug_effect(
                            static_cast<common::genotype_t>(this->pg_type_of[pp]),
                            this->drug_of[ss],
                            this->pg_stage_of[pp]
                        );
...
void BloodSystemManager::parasite_mutation(
        common::GenotypeManager* const gt_mgr,
        common::DrugManager* const dg_mgr,
        int current_time_step
    ) {
    
    if (!this->kMutation_enabled) {return;}
    if (current_time_step < this->kMutation_start_at_time_step) {return;}

    if (this->kFunc_parasite_mutation_version == 3) {
        this->parasite_mutation_tact(
            gt_mgr,
            dg_mgr
            // current_time_step
        );
    } else if (this->kFunc_parasite_mutation_version == 0) {
        ...
        std::cout << "Unknown kFunc_parasite_mutation_version : "
                    << this->kFunc_parasite_mutation_version << std::endl;
        throw std::logic_error(
                "Exception: Undefined behaviour by kFunc_parasite_mutation_version value"
            );
    }
}

void BloodSystemManager::parasite_mutation_tact(
        common::GenotypeManager* const gt_mgr,
        common::DrugManager* const dg_mgr
        // int current_time_step
    ) {

    // std::cout << "parasite_mutation_tact\n";

    for(auto& ii : this->log_new_fixation) {ii = 0;}
    for(auto& ii : this->log_new_mutation) {ii = 0;}

    for (int ss = 0; ss < this->sum_num_systems; ss++) {
        if (this->head_pg_of[ss] < 0) {
            continue; // nothing in this host
        }
        if (this->most_advanced_stage_of[ss] <= common::StageName::kLiver) {
            continue; // no mutation in liver
        }

        const int kLbound = ss * kNum_pgs_per_system;
        for (int pp = kLbound + this->head_pg_of[ss]; pp <= kLbound + this->tail_pg_of[ss]; pp++) {

            if ( (this->pg_stage_of[pp] == common::StageName::kBlood)
                    || (this->pg_stage_of[pp] == common::StageName::kInfectious)
                ) {

                common::genotype_t orig_gt = static_cast<common::genotype_t>(
                        this->pg_type_of[pp]
                    );
                common::genotype_t prop_gt = orig_gt;
                int num_mut_in_prop = gt_mgr->fun_mutation_combined(
                        prop_gt // proposed genotype
                    );

                if (num_mut_in_prop == 0) {
                    // no mutation proposed
                } else {

                    this->log_new_mutation[static_cast<int>(prop_gt)]++;
                    
                    int diff_in_drug_pressure = 0;
                    if (this->drug_of[ss] == common::DrugName::kNoDrug) {
                        diff_in_drug_pressure = 0;
                    } else {
                        diff_in_drug_pressure = dg_mgr->compare_drug_effect_rate(
                                                    orig_gt,
                                                    prop_gt,
                                                    this->drug_of[ss]
                                                );
                    ...
                    bool if_fixation = false;
                    switch (diff_in_drug_pressure) {
                        case 0 :
                            if_fixation =
                                util::get_rand_uniform() < this->kPer_infection_fixation_base_rate;
                            break;
                        case 1 :
                            if_fixation = false;
                            break;
                        case -1 :
                            ...
                            break;
                        default :
                            std::cout << "diff_in_drug_pressure = " << diff_in_drug_pressure << "\n";
                            throw std::logic_error(
                                    "Exception: Undefined behaviour by diff_in_drug_pressure value"
                                   );
                    }

                    if (if_fixation) {
                        this->pg_type_of[pp] = static_cast<common::ParasiteType>(prop_gt);
                        this->log_new_fixation[static_cast<int>(prop_gt)]++;
                    }
                }
...
void BloodSystemManager::parasite_mutation_mmc_wp2_drug_days_fixation_per_site(int current_time_step) {
    ...
    for (int ss = 0; ss < this->sum_num_systems; ss++) {
        ...
        // std::cout << "this->drug_of[ss]:" << this->drug_of[ss] << std::endl;
        // std::cout << "applicable_mutation_rate:" << applicable_mutation_rate << std::endl;
        // if (applicable_mutation_rate > 0.00001) {
        //     std::cin.ignore();
        // }
...
bool BloodSystemManager::parasite_stage_progression_probability_based(
    ...
    switch (this->pg_stage_of[p_index]) {
        ...
        case common::StageName::kInfectious :  {// natural recovery
                ...
                // std::cout << "applicable_natural_recovery_rate: " << applicable_natural_recovery_rate << "\n";
                if (this->pg_type_of[p_index] != common::ParasiteType::kR0) { // pg carries resistance
                // if (
                //     (this->pg_type_of[p_index] == common::ParasiteType::kRa)
                //     || (this->pg_type_of[p_index] == common::ParasiteType::kRab) ) { // pg carries RA resistance
                    applicable_natural_recovery_rate *= (1.0 + this->kNatural_recovery_resistance_cost);
                    
                    // std::cout << "this->pg_type_of[p_index]: " << this->pg_type_of[p_index] << "\n";
                    // std::cout << "applicable_natural_recovery_rate: " << applicable_natural_recovery_rate << "\n";
                    // std::cin.ignore();
                }
                // if (this->pg_type_of[p_index] != common::ParasiteType::kR0) {
                // }
...
void BloodSystemManager::parasite_dominance(
    ...
    for (int ss = 0; ss < this->sum_num_systems; ss++) {
        ...
        if (this->head_pg_of[ss] < 0){
            ...
            if (this->kFunc_dominance_version == 2) {
                this->parasite_dominance_tact_kernel(
            ...
            } else if (this->kFunc_dominance_version == 0) {
                ...
                std::cout << "Unknown kFunc_dominance_version: "
                            << this->kFunc_dominance_version << std::endl;
                throw std::logic_error(
                    "Exception: Undefined behaviour by kFunc_dominance_version value"
                );
            }
        }
...
void BloodSystemManager::step_report_genotype_weights(){
    this->step_report_genotype_weights(false);
}

void BloodSystemManager::step_report_genotype_weights(
        bool force_run_this_call
    ) {
    if (!this->kReport_genotype_weights && !force_run_this_call) {return;}
    ...
    // this->daily_record_of_num_r0_pgs  = 0;
    // this->daily_record_of_num_ra_pgs  = 0;
    // this->daily_record_of_num_rb_pgs  = 0;
    // this->daily_record_of_num_rab_pgs = 0;
    ...
    for (int ss = 0; ss < this->sum_num_systems; ss++) {
        ...
        if (this->is_infected(ss)) {
            ...
            int ss_pg_count = this->pg_count(ss);
            this->daily_record_of_num_pgs += ss_pg_count;
            ...
            int head = this->head_pg_idx(ss);
            int tail = this->tail_pg_idx(ss);
            ...
            for (int pp = head; pp <= tail; pp++) {
                this->daily_record_of_genotype_weights[static_cast<int>(this->pg_type_of[pp])]
                    += 1.0/static_cast<float>(ss_pg_count);
            }
    ...
    this->daily_record_of_gw_per_site = common::GenotypeManager::sum_vector_on_sites(
        this->daily_record_of_genotype_weights,
        this->list_of_single_site_pts
    );

    // this->daily_record_of_gw_76T  = common::GenotypeManager::sum_vector_on_sites(
    //                 this->daily_record_of_genotype_weights, common::ParasiteType::kTNY__C1
    //                 ) / this->daily_record_of_parasite_positive_systems;
    // this->daily_record_of_gw_86Y  = common::GenotypeManager::sum_vector_on_sites(
    //                 this->daily_record_of_genotype_weights, common::ParasiteType::kKYY__C1
    //                 ) / this->daily_record_of_parasite_positive_systems;
    // this->daily_record_of_gw_184F = common::GenotypeManager::sum_vector_on_sites(
    //                 this->daily_record_of_genotype_weights, common::ParasiteType::kKNF__C1
    //                 ) / this->daily_record_of_parasite_positive_systems;
    // this->daily_record_of_gw_SndC = common::GenotypeManager::sum_vector_on_sites(
    //                 this->daily_record_of_genotype_weights, common::ParasiteType::kKNYNYC1
    //                 ) / this->daily_record_of_parasite_positive_systems;
    // this->daily_record_of_gw_580Y = common::GenotypeManager::sum_vector_on_sites(
    //                 this->daily_record_of_genotype_weights, common::ParasiteType::kKNY__Y1
    //                 ) / this->daily_record_of_parasite_positive_systems;
    // this->daily_record_of_gw_2x   = common::GenotypeManager::sum_vector_on_sites(
    //                 this->daily_record_of_genotype_weights, common::ParasiteType::kKNY__C2
    //                 ) / this->daily_record_of_parasite_positive_systems;

    // float gw = common::GenotypeManager::sum_vector_on_sites(
    //     this->daily_record_of_genotype_weights, common::ParasiteType::kKNY__Y1
    // );
    // this->daily_record_of_gw_580Y = gw/this->daily_record_of_parasite_positive_systems;
...
void BloodSystemManager::parasite_dominance_tact_kernel(
    ...
    std::vector<common::ParasiteType> list_of_candidate_pts;
    float ss_lowest_efficacy = 1.0;

    for (int pp = head_pg_index; pp <= tail_pg_index; pp++) {
        
        common::StageName pp_stg = pg_stage_of_array[pp];
        common::ParasiteType pp_pt = pg_type_of_array[pp];


        assert(pp_stg != common::StageName::kNotInSystem);
        assert(pp_pt  != common::ParasiteType::kNoParasite);
        
        if (pp_stg > most_advanced_stage_of_array[system_index]) {
            most_advanced_stage_of_array[system_index] = pp_stg;
        }

        if (pp_stg == common::StageName::kInfectious) {

            float pp_efficacy = dg_mgr->get_drug_effect_rate(
                                        static_cast<common::genotype_t>(pp_pt),
                                        ss_drg
                                    );
            if (pp_efficacy < ss_lowest_efficacy) {
                ss_lowest_efficacy = pp_efficacy;
                list_of_candidate_pts.clear();
                list_of_candidate_pts.push_back(pp_pt);
            } else if (pp_efficacy == ss_lowest_efficacy) {
                list_of_candidate_pts.push_back(pp_pt);
            }
        }
    }

    size_t num_candidates = list_of_candidate_pts.size();
    if (num_candidates == 1) {
        dominant_type_of_array[system_index] = list_of_candidate_pts[0];
    } else if (num_candidates > 1) {
        dominant_type_of_array[system_index] = list_of_candidate_pts[
                util::get_random_index_from_vector_of_size(num_candidates)
            ];
    }
...
void BloodSystemManager::parasite_dominance_mmc_wp2_kernel(
    ...
    // infectious and has at least one resistance type, check drugs
        ...
        //     // std::cout << "R0,";
        // }
        // if(any_infectious_ra) {
        //     list_of_infectious_resistant_pts.push_back(common::ParasiteType::kRa);
        //     // std::cout << "Ra,";
        // }
        // if(any_infectious_rb) {
        //     list_of_infectious_resistant_pts.push_back(common::ParasiteType::kRb);
        //     // std::cout << "Rb,";
        // }
        // if(any_infectious_rab) {
        //     list_of_infectious_resistant_pts.push_back(common::ParasiteType::kRab);
        //     // std::cout << "Rab,";
        // }

        // std::random_shuffle(list_of_infectious_resistant_pts.begin(), list_of_infectious_resistant_pts.end());
        ...
        // if (list_of_infectious_resistant_pts.size()>2){
        //     for (auto pt : list_of_infectious_resistant_pts) {
        //         std::cout << pt << "\n";
        //     }
        //     std::cout << "\nPicked:" << dominant_type_of_array[system_index] << "\n";
        //     std::cin.ignore();
        // }
    ...
    for(auto pp : util::Enum_iterator<common::ParasiteType>()) {
        ...
        for(auto dd : util::Enum_iterator<common::DrugName>()) {
            ...
            for(auto ss: util::Enum_iterator<common::StageName>()) {
                ...
                throw std::logic_error("bm.print_drug_parameters() not in use");
                // std::cout << "kPds["
                //             << pp << "]["
                //             << dd << "]["
                //             << ss << "]="
