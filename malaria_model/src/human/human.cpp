...
HumanManager::HumanManager (
    ...
    if (initial_population<1) {
        ...
        this->is_forest_goer_of = new bool[initial_population];
        this->is_plantation_worker_of = new bool[initial_population];
        ...
        for (int hh = 0; hh < initial_population; hh++) {
            ...
            this->is_forest_goer_of[hh] = false;
            this->is_plantation_worker_of[hh] = false;
    ...
    this->journey_plan_of.resize(this->sum_num_humans);
...
HumanManager::~HumanManager() {
    ...
    if (is_forest_goer_of) {delete[] is_forest_goer_of;}
    if (is_plantation_worker_of) {delete[] is_plantation_worker_of;}
...
void HumanManager::reset_human_to_birth(const int h_index) {
    ...
    // this is not updated to VM unless mobility is enabled
    // i.e. assuming same attractiveness as previous human
