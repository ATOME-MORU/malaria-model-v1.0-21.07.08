...
Simulator::Simulator (
        ...
        vll_mgr(
            ...
            config["treatment"]["malaria_post"]["rate_reduction"]["enabled"].GetBool(),
            config["treatment"]["malaria_post"]["rate_reduction"]["size"].GetFloat(),
            config["treatment"]["malaria_post"]["rate_reduction"]["start_on_day"].GetInt(),
            config["treatment"]["malaria_post"]["rate_reduction"]["slope_days"].GetInt(),
            config["treatment"]["malaria_post"]["rate_reduction"]["length_days"].GetInt(),

            config["treatment"]["mda_treatment_rate"].GetFloat(),
            config["treatment"]["mda_max_age"].GetInt(),
            config["treatment"]["mda_min_age"].GetInt(),
            
            config["treatment"]["mmc_wp2"]["coverage"].GetFloat(),
            config["treatment"]["mmc_wp2"]["drug"].GetInt(),

            config["treatment"]["tact"]["coverage"].GetFloat(),
            config["treatment"]["tact"]["first_line_drug_a"].GetInt(),
            config["treatment"]["tact"]["first_line_drug_b"].GetInt(),
            config["treatment"]["tact"]["switch_day"].GetInt(),
            config["treatment"]["tact"]["first_line_increase"].GetBool(),
            config["treatment"]["tact"]["first_line_max"].GetFloat(),
            
            config["treatment"]["tact"]["a7"]["enabled"].GetBool(),
            config["treatment"]["tact"]["a7"]["annual_inc"].GetFloat()/static_cast<float>(common::kNum_days_in_one_year),

            config["treatment"]["tact"]["a11"]["enabled"].GetBool(),
            config["treatment"]["tact"]["a11"]["init_tact_rate"].GetFloat(),
            config["treatment"]["tact"]["a11"]["num_years"].GetFloat(),


            config["treatment"]["tact"]["use_private_drugs"].GetBool(),

            config["treatment"]["forest"]["coverage"].GetFloat(),
            ...
            config["mosquito"]["seasonality"]["data_file"]["horizontal_shift_days"].GetInt(),
            config["mosquito"]["seasonality"]["base_percent"].GetFloat(),
            ...
            config["mobility_forest"]["enabled"].GetBool(),
            config["mobility_forest"]["precedence_over_static_mobility"].GetFloat(),
            config["mobility_forest"]["forest_set_off_probability"].GetFloat(),
            config["mobility_forest"]["forest_return_home_probability"].GetFloat(),
            config["mobility_forest"]["stay_at_entry_points"].GetBool(),
            config["mobility_forest"]["return_stay_at_entry_points"].GetBool(),
            config["mobility_forest"]["func_mobility_forest"]["version"].GetInt(),
            config["mobility_forest"]["v1"]["return_early"].GetBool(),
            config["mobility_forest"]["v1"]["min_num_way_points"].GetInt(),
            config["mobility_forest"]["v2"]["follow_routes"].GetBool(),
            config["mobility_forest"]["ban"]["from"].GetInt(),
            config["mobility_forest"]["ban"]["length"].GetInt(),
            config["mobility_forest"]["ban"]["compliance"].GetFloat(),

            config["human"]["attractiveness"]["use_sampler_list"]["enabled"].GetBool(),
            config["human"]["attractiveness"]["use_sampler_list"]["unit_size"].GetInt()
        ...
        mda(
            ...
            config["intervention"]["mda"]["fixed_routes"].GetBool(),
        ...
        fmda(
            ...
            config["intervention"]["fmda"]["fixed_routes"].GetBool(),
        ...
        tmda(
            ...
            config["intervention"]["tmda"]["fixed_routes"].GetBool(),
        ...
        bsys_mgr(
            ...
            config["drug"]["resistance_multipliers"]["drug_b_effect_on_ra"].GetFloat(),
            config["drug"]["resistance_multipliers"]["drug_ab_effect_on_ra"].GetFloat(),
            config["drug"]["resistance_multipliers"]["drug_a_effect_on_rb"].GetFloat(),
            ...
            config["genotype"]["mutation"]["enabled"].GetBool(),
            config["genotype"]["mutation"]["start_at_time_step"].GetInt(),
            ...
            config["genotype"]["mutation"]["func_parasite_mutation"]["per_infection_fixation_base_rate"].GetFloat(),

            config["simulation"]["reports"]["genotype_weights"].GetBool(),
        ...
        msq_dp(
            new village::MosquitoDP(
                config["mosquito"]["dispersion"]["enabled"].GetBool(),
                config["mosquito"]["dispersion"]["radius"].GetFloat(),
                config["mosquito"]["dispersion"]["strength"].GetFloat(),
                config["mosquito"]["dispersion"]["individual_based"].GetBool(),

                config["mosquito"]["dispersion"]["retaining_strength_human_presence_each"].GetFloat(),
                config["mosquito"]["dispersion"]["retaining_strength_human_presence_max"].GetFloat(),
                config["mosquito"]["dispersion"]["retaining_strength_spawn_point_each"].GetFloat(),
                config["mosquito"]["dispersion"]["retaining_strength_spawn_point_max"].GetFloat(),

                config["mosquito"]["dispersion"]["pit"]["enabled"].GetBool(),
                config["mosquito"]["dispersion"]["pit"]["lost_prob"].GetFloat(),
                config["mosquito"]["dispersion"]["pit"]["found_prob"].GetFloat(),

                this->vll_mgr.get_village_distances(),
                this->vll_mgr.get_mosquito_spawning_site_size_of(),
                this->vll_mgr.get_ids_of_forest_pois()
            )
        ),
        gt_mgr(
            new common::GenotypeManager(
                std::vector<float>(
                    config["genotype"]["mutation"]["num_sites"].GetInt(),
                    config["genotype"]["mutation"]["base_rate"].GetFloat()
                )
            )
        ),
        dg_mgr(
            new common::DrugManager(
                config["drug"]["efficacy"]["file"].GetString(),
                config["drug"]["loss"]["file"].GetString(),
                true // verbose
            )
        ),
        rpt_mgr(
            ...
            config["simulation"]["reports"]["mobility_daily"].GetBool(),
            config["simulation"]["reports"]["mobility_daily_od"].GetBool(),
            config["simulation"]["reports"]["mosquito_dist_daily"].GetBool(),
            config["simulation"]["reports"]["mobility_daily_intensity"].GetBool(),


            config["simulation"]["plots"]["prevalence_daily_y_max"].GetInt(),
            config["drug"]["failure_if_clinical_on_day"].GetInt(),
            static_cast<size_t>(config["drug"]["failure_measurement_length"].GetInt()),
        ...
        std::cout << "Sim: All objects initialised." << std::endl;
        ...
        this->gt_mgr->print_all_combined_probabilities();
        ...
        if (this->config["simulation"]["init_infections_from_agestruct"].GetBool()) {
            ...
            if (this->config["within_host"]["init_file"]["use_old_format"].GetBool()) {
                ...
                if (this->config["within_host"]["init_file"]["ra_rate"].GetFloat() > 0.0) {
                    ...
                    this->bsys_mgr.step_report_genotype_weights(true);
                    ...
                    this->bsys_mgr.parasite_dominance(this->dg_mgr);
        ...
        //////////////////////////////////////////////////////////////////////
        // Infection initial composition

        std::cout << "Sim: init parasite type composition ...";

        if (this->config["infection"]["init"]["composition"]["enabled"].GetBool()) {

            const rapidjson::Value& config_gt_list = 
                        this->config["infection"]["init"]["composition"]["genotype_list"];
            const rapidjson::Value& config_wt_list = 
                        this->config["infection"]["init"]["composition"]["weight_list"];
            
            assert(config_gt_list.IsArray());
            assert(config_wt_list.IsArray());
            assert(config_gt_list.Size() == config_wt_list.Size());

            std::vector<common::ParasiteType> pt_sample_list;

            for (rapidjson::SizeType ss = 0; ss < config_wt_list.Size(); ss++) {
                for (int ii = 0; ii < config_wt_list[ss].GetInt(); ii++) {
                    pt_sample_list.push_back(
                        common::GenotypeManager::map_str_to_pt(
                            config_gt_list[ss].GetString()
                            )
                        // static_cast<common::genotype_t>(config_gt_list[ss].GetInt())
                    );
                }
            }

            this->bsys_mgr.reassign_parasitetypes(pt_sample_list);
            this->vll_mgr.reassign_parasitetypes_mosq(pt_sample_list);
    ...
    // Mosquito Manager
        ...
        // this->msq_dp = new village::MosquitoDP(
        //     config["mosquito"]["dispersion"]["enabled"].GetBool(),
        //     config["mosquito"]["dispersion"]["radius"].GetFloat(),
        //     config["mosquito"]["dispersion"]["strength"].GetFloat(),
        //     config["mosquito"]["dispersion"]["individual_based"].GetBool(),
        ...
        std::cout << "Sim: initialisation completed.\n";
}
...
Simulator::~Simulator() {
    ...
    if (this->dg_mgr) {delete this->dg_mgr;}
    if (config["mosquito"]["ibm"]["in_use"].GetBool()) {
...
void Simulator::run(){
    ...
    int performance_timing_start = std::min(
        this->config["simulation"]["performance"]["timing_start"].GetInt(),
        this->num_time_steps
        );
    int performance_timing_finish = std::min(
        this->config["simulation"]["performance"]["timing_finish"].GetInt(),
        this->num_time_steps
        );
    ...
    const bool kMosq_method_using_ode = config["mosquito"]["ode"]["in_use"].GetBool();
    const bool kMosq_method_using_ibm = config["mosquito"]["ibm"]["in_use"].GetBool();

    const bool kBm_injection_enabled = this->config["infection"]["injection"]["enabled"].GetBool();
    const int kBm_injection_at_time_step = this->config["infection"]["injection"]["at_time_step"].GetInt();
    const common::ParasiteType kBm_injection_type = common::GenotypeManager::map_str_to_pt(
                config["infection"]["injection"]["type"].GetString()
            );

    const bool kBm_import_enabled = config["infection"]["import"]["enabled"].GetBool();
    const float kBm_import_rate = config["infection"]["import"]["rate"].GetFloat();
    const common::ParasiteType kBm_import_type = common::GenotypeManager::map_str_to_pt(
                config["infection"]["import"]["type"].GetString()
            );
    const int kBm_import_start_on_day = config["infection"]["import"]["start_on_day"].GetInt();

    // bool if_threading = false;
    // std::thread thrd_rnd_nmbrs_drug_loss;

    const bool kSim_perf_report_per_step = config["simulation"]["performance"]["report_per_step"].GetBool();
...
//SOD   ///// Mandatory start of day processes
        ...
        // std::cout << "[6]\n";
            ...
            // if (config["genotype"]["mutation"]["enabled"].GetBool() && tt >= config["genotype"]["mutation"]["start_at_time_step"].GetInt()) {
                ...
                this->bsys_mgr.parasite_mutation(
                    this->gt_mgr,
                    this->dg_mgr,
                    tt
                );
            ...
            this->bsys_mgr.parasite_dominance(this->dg_mgr); // prevalence and clinical stats done here
...
// }
                ...
                this->msq_dp->simple_dispersion(
                    this->vll_mgr.get_mosquito_infected_composition_of(),
                    this->vll_mgr.get_population_of(),
                    false
                );
                this->msq_dp->simple_dispersion(
                    this->vll_mgr.get_mosquito_infectious_composition_of(),
                    this->vll_mgr.get_population_of(),
                    true
                );
                ...
                this->msq_dp->mosquito_survival_step(
                    this->vll_mgr.get_mosquito_death_probability_infected(),
                    this->vll_mgr.get_mosquito_death_probability_infectious()
                );
                this->msq_dp->mosquito_incubation_step(
                    this->vll_mgr.get_mosquito_incubation_probability()
                );
        ...
        ////// Infection Injection
        if (kBm_injection_enabled) {
            if ( kBm_injection_at_time_step == tt ) {

                std::cout << "[*] Injection ...\n";

                float wanted_perc = config["infection"]["injection"]["size_as_in_percentage_of_positive_population"].GetFloat();

                if (wanted_perc > 0.0) {

                    this->bsys_mgr.step_report_genotype_weights(true);

                    int wanted_count = this->bsys_mgr.daily_record_of_parasite_positive_systems
                        * wanted_perc;

                    int existing_count = this->gt_mgr->sum_vector_on_sites(this->bsys_mgr.daily_record_of_genotype_weights, kBm_injection_type);
                    // ^^ daily_record_of_genotype_weights/daily_record_of_parasite_positive_systems is genotype frequency

                    float sum_gw = this->gt_mgr->sum_vector_on_sites(
                                        this->bsys_mgr.daily_record_of_genotype_weights,
                                        common::ParasiteType::kWild
                                    )/static_cast<float>(this->bsys_mgr.daily_record_of_parasite_positive_systems);
                    // std::cout << "sum_gw:" << sum_gw << "\n";
                    // std::cin.ignore();
                    assert(sum_gw - 1.0 < 0.000001);


                    if (wanted_count > existing_count) {
                        this->bsys_mgr.parasite_injection(
                            kBm_injection_type,
                            wanted_count - existing_count
                        );
                    }
        ...
        ////// Importation
        if (kBm_import_enabled && tt >= kBm_import_start_on_day) {
            this->bsys_mgr.report_daily_imported_cases = 0;
            if (util::get_rand_uniform() < kBm_import_rate) {
                this->bsys_mgr.parasite_importation(
                    1, kBm_import_type, tt
                );
                this->bsys_mgr.report_daily_imported_cases++;
            }
        }
        ...
        //std::cout << "[Step.end] ...\n";
            if ((!config["mosquito"]["ode"]["in_use"].GetBool())
                ...
                this->vll_mgr.update_summary(
                    this->msq_dp->get_num_infected(),
                    this->msq_dp->get_num_infectious(),
                    this->msq_dp->log_step_mosq_pit_death_infected,
                    this->msq_dp->log_step_mosq_pit_death_infectious,
                    this->msq_dp->log_step_mosq_pit_incubation
                );
            }
