...
TEST_CASE("124.1: Step_mobility Functions: step_mobility", "[step_mobility:stay-home-between-mobilities]" ) {

    /*
    This test verifies that mobilites (village <--> forest,  village <--> village) conform to expected rules.
    
    A forest_goer agent can move from home to the village only through the existing forest route and must return to the home village
    before going on another forest trip. 
    Furthermore, if the route is composite (involved more than one forestPOI) the agent stay only one single day/night in the transient 
    forestPOIs or forestEntry if 'stay_at_entry_points' option is true.

    It is in the final forestPOI that agents can stay more than one night. 

    If 'stay_at_entry_points' is true, the agent stays one day/night in a forestEntry (e), and we only check for the taken forest route
    is correct/valid. Currently, we do not check whether a given forestEntry matches the valid forest routes from that entry
    (in other words, if the forestPOI route is correct, it is unchecked whether the right forestEntry point has been used). 

    There is also a settable min_num_way_points parameter. This is to invalidate routes/paths in the forest that are too short.
    For example, changing  'min_num_way_points' from from 1 to 2, invalidate those paths that only have one forest_poi point, 
    unless the single forest_poi is the only remaining path to the entry point (every single entry point retains the only 
    existing route/path to forest, even if it should be normally be invalided by a given  'min_num_way_points').
    
    The test automatically use validated routes/paths determined by  "min_num_way_points" These are printed at the beginning of
    the test outputs. 

    If an agent does a static move (i.e. from one village to another village), the rule stipulates that the agent does not move again 
    before returning home first.

    Please note if a 'static move' takes place by an agent, say from village 0 to village 2, and then the agent stays 
    several days/nights in v2 but the simulations terminate before knowing whether the agent returned home or not, 
    such a move [e.g. v0(3 days), v2(10 days)] is not be flagged as an error. 

    The test makes sure every agent conform to these rules, otherwise, the boolean error flag will be turned on 
    and the final REQUIRE will fail.

    The checking for the conformance to these rules for no static moves is currently done only for "pathing/routed" option.
    In other words, the no static moves for "In-out" and "Random-walking" are not currently verified.
   
    For the "pathing/routed" option, in addition to checking the possible route, there is another extra checking which verifies the move from one location to another is possible 
    (otherwise, a second error flag is raised and the second REQUIRE will not pass).
        
    The test generates a simple simulation by calling the new VillageManager 'step_mobility' function at desirable and settable  the number of days (e.g. 30 or 360 days, etc.)
    
    This can be done using any region file.
    
    The test can be also done by choosing whether the agents 'stay_at_entry_points' or not as well as 'min_num_way_points'.  

    The choice of forest mobility type is done by setting the value of func_mobility_forest_version.
    This can be 0 ("In-out"), 1 ("Pathing/routed", 2 ("Random Walking"))
    
    Some additional stats are also printed, including the last longest days spent in forestPOI and no forest location.
    ...
    */
    ...
    //const int func_mobility_forest_version = 0; //In-out
    const int func_mobility_forest_version = 1;  //pathing-routed
    //const int func_mobility_forest_version = 2; //random-walk
    ...
    village::VillageManager vll_mgr(
            ...
            func_mobility_forest_version, //1,  // const int func_mobility_forest_version
    ...
    std::cout << "total locations (villages + forest entries + POIs): " <<  vll_mgr.sum_num_villages<<"\n";

    int nb_of_villages_only = 0;
    int nb_of_f_entries_only = 0;
    int nb_of_f_poi_only = 0;

    std::vector<int> locations_ids = vll_mgr.get_village_ids_ordered_by_tc_desc();
    for (unsigned int ii = 0; ii < locations_ids.size(); ii++) {
        if (vll_mgr.get_location_type(locations_ids.at(ii)) == common::LocationType::kVillage) 
            nb_of_villages_only++;
        if (vll_mgr.get_location_type(locations_ids.at(ii)) == common::LocationType::kForestEntry) 
            nb_of_f_entries_only++;
        if (vll_mgr.get_location_type(locations_ids.at(ii)) == common::LocationType::kForestPOI) 
            nb_of_f_poi_only++;     
    }       

    int index_offset = nb_of_villages_only;

    std::cout << "total villages only: " <<  nb_of_villages_only <<"\n";
    std::cout << "total f_entries only: " <<  nb_of_f_entries_only <<"\n";
    std::cout << "total f_POIs only: " <<  nb_of_f_poi_only <<"\n";

    util::VillageGraph *fg = vll_mgr.build_forest_graph(
        vll_mgr.distances,
        vll_mgr.ids_of_forest_entry_points,
        vll_mgr.ids_of_forest_pois,
        true
        );

    std::cout << "\nEdge weigts between forest entries and POIs (possible moves): \n";
    for (int ii = 0; ii < nb_of_f_entries_only+ nb_of_f_poi_only; ii++)
    {
        for (int jj = 0; jj <  nb_of_f_entries_only+ nb_of_f_poi_only; jj++)
            std::cout << fg->get_edge_weight(ii, jj) << ", ";
        std::cout << "\n";
    }
    std::cout << "\n";
    ...
    std::cout << "\nValide routes for following forest entries (e) are: \n";
    ...
    for (unsigned int ii=0; ii<forest_route_network_lists.size(); ii++) {
        std::cout << "\t";
         for (unsigned int jj=0; jj<forest_route_network_lists.at(ii).size(); jj++) {
             std::cout << forest_route_network_lists.at(ii).at(jj) << ", ";
         }
        std::cout << "\n";
    } */
    ...
    bool stay_home_between_mobilities = true; // checking done for pathing/routed

    bool link_between_locations_exist = true; // checking done for all (pathing/routed, random walk, in-out)
            ...
            if (agents_stats.at(hh).size()>1) {
                ...
                //process journey route: 
                    ...
                    if (move_static) {
                      ...
                      if (func_mobility_forest_version == 1) { //for now, we only check the validity for pathing/routed
                            bool route_exist = false; // this is used for pathing/routed option 
                            bool stayed_only_one_day_in_transient_forest_routes = true;

                            for (unsigned int ii=0; ii<out_of_home_route.size()-1; ii++) { ///+++++++++++++++
                                    int i = out_of_home_route.at(ii)-index_offset;
                                    int j = out_of_home_route.at(ii+1)- index_offset;
                                    //std::cout << "[" <<i << "," << j <<"]=" <<fg->get_edge_weight(i,j)  << "; ";    
                                    if (fg->get_edge_weight(i,j) ==0) {
                                        link_between_locations_exist = false;
                                        std::cout << ">>>>> Error: link between locations do not exist: [" <<i+index_offset << "," << j+index_offset <<"]=" <<fg->get_edge_weight(i,j)  << "\n"; 
                                    }
                            }
                            ...
                            for (unsigned int ii=0; ii<out_of_home_route_days.size()-1; ii++) {
                                if (out_of_home_route_days.at(ii) != 1) // when stay_at_entry_points is true, we also check that agents can stay there only 1 night
    ...
    std::cout << "\n \n===============================================================================\n";
    std::cout << "NOTE that checking the validity of journeys from one location to another is currenlty done only for pathing/routed (i.e. func_mobility_forest_version == 1) \n";
    std::cout << "This test was exectuted with func_mobility_forest_version == " << func_mobility_forest_version;

    if (func_mobility_forest_version==0)
        std::cout << " (In-out) \n";
    if (func_mobility_forest_version==1)
        std::cout << " (Pathing/Routed) \n";
    if (func_mobility_forest_version==2)
        std::cout << " (Random Walking) \n";
    //std::cout << "\n";
    
    REQUIRE(stay_home_between_mobilities == true);

    REQUIRE(link_between_locations_exist == true);
