...
TEST_CASE( "50.1: Treatment Functions: treatment_by_malaria_post_kernel", "[village:treatment_by_malaria_post_kernel]" ) {
    ...
    for (auto& ii: daily_record_of_new_treatments_by_parasite_type) {
        ii = 0;  //to set reasonable value (to check)
    }

    const float treatment_rate_reduction_size= 1.0;  //to set reasonable value  (to check)
...
TEST_CASE( "50.2: Treatment Functions: treatment_by_mda_kernel", "[village:treatment_by_mda_kernel]" ) {
    ...
    const int kMda_max_age=80; // set reasonable value
    const int kMda_min_age=10; // set reasonable value
    int first_age_value = 10; // set reasonable value
    int* human_age_of = &first_age_value;  // set reasonable value
