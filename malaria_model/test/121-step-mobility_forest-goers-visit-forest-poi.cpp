...
TEST_CASE("121.1: Step_mobility Functions: step_mobility", "[step_mobility:only_forest_goer_visit_forestPOI]" ) {

    /*
    This test is desinged to verify that only forest-goer agents visits forestPOI.
    The test can be done using different number of simulated days (e.g. 1 month/ 1 year, etc)
    and using differnt 'region' file (including list of different villages, and forest POI)
    */
   ...
   //===========================
    ...
    bool only_forest_goer_visited_forest_poi = true;
    
    std::cout << "\n ========== BEFORE call to step_mobility/ simulated days start/:" << "\n";
        ...
        for (int vv = 0; vv < vll_mgr.sum_num_villages; vv++){      
            for (unsigned int ii = 0; ii < vll_mgr.at_village_register.at(vv).size(); ii++){
                ...
                if (vll_mgr.get_location_type(vv) == common::LocationType::kForestPOI) {    
                    //std::cout << "forestPOI ("<< vv<< "): agent visited, id: " << agentID << "\n";                       
                    if (!hmn_mgr.get_is_forest_goer(agentID)) {
                        std::cout << "***** WAS NOT forest-goer: " << agentID << "\n";
                        only_forest_goer_visited_forest_poi = false;
                    }
                }
            }
        }
    ...
    REQUIRE(only_forest_goer_visited_forest_poi == true);
}