...
TEST_CASE( "21.7: Randomness: get_random_index_from_vector_of_size", "[util_rand][util_rand_pick_index]" ) {
    
    std::cout << "Testing util::get_random_index_from_vector_of_size\n";

    const int kNum_tests = 10000000;
    std::vector<size_t> size_list{1,2,3,4,5,10,100};

    for (size_t ss: size_list) {
        std::cout << "\tsize" << ss << ":\n";
        std::vector<int> v(ss, 0);
        for (int tt = 0; tt < kNum_tests; tt++) {
            v[util::get_random_index_from_vector_of_size(ss)]++;
        }
        for (int cc : v) {
            if (ss < 10) {
                std::cout << "\t\t" << cc << "\n";
            }
            Approx target = Approx(kNum_tests/ss).epsilon(0.01);
            REQUIRE(cc == target);
        }
    }
}
