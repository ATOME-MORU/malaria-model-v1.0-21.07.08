...
class ReportManager {
    ...
    const std::vector<std::string> kReport_file_names = {
        ...
        "_mobility_daily.csv",           // 11
        "_mobility_daily_od.csv",        // 12
        "_mosquito_dist_daily.csv",      // 13
        "_mobility_daily_intensity.csv"  // 14
        // "_village_daily.csv",
    ...
    // mobility (origin-destination report)
    std::vector<int> last_known_location_of;
...
public:
    ...
    void daily_report_mobility(
        int sim_time,
        const human::HumanManager* h_mgr,
        std::ofstream* os
    );

    void daily_report_mobility_od(
        int sim_time,
        const human::HumanManager* h_mgr,
        std::ofstream* os
    );

    void daily_report_mosquito_dist(
        int sim_time,
        std::ofstream* os
    );

    void daily_report_mobility_intensity(
        int sim_time,
        std::ofstream* os
    );
    ...
    static float daily_report_drug_fail_rate_calc(
        float today_in,
        float today_fail,
