...
class BloodSystemManager {
    ...
    const bool kMutation_enabled;
    const int kMutation_start_at_time_step;
    ...
    const float kPer_infection_fixation_base_rate;

    const bool kReport_genotype_weights;
...
float kPds_kill_probability[static_cast<int>(common::ParasiteType::Last)+1]
    ...
    // B. Piperaquine
        ...
        // {0,0,1.0/5.0,       1.0/5.0}, // mmc_wp2_rev18, SA1-4, A4, PPQ
        // {0,0,1.0/5.0,       1.0/5.0}, // mmc_wp2_rev18, SA1-4, A5, AQ
        {0,0,1.0/7.0,       1.0/7.0}, // mmc_wp2_rev18, SA1-4, A6, L
    // A+B. Piparte
...
public:
    ...
    // float daily_record_of_gw_2x = 0.0;
    // float daily_record_of_gw_580Y = 0.0;
    // float daily_record_of_gw_SndC = 0.0;
    // float daily_record_of_gw_184F = 0.0;
    // float daily_record_of_gw_86Y = 0.0;
    // float daily_record_of_gw_76T = 0.0;

    const std::vector<common::ParasiteType> list_of_single_site_pts{
        common::ParasiteType::kKNY__C2,
        common::ParasiteType::kKNY__Y1,
        common::ParasiteType::kKNYNYC1,
        common::ParasiteType::kKNF__C1,
        common::ParasiteType::kKYY__C1,
        common::ParasiteType::kTNY__C1
    };

    std::vector<float> daily_record_of_gw_per_site{
        std::vector<float>(list_of_single_site_pts.size(), 0.0)
    };

    int daily_record_of_num_pgs = 0;
    float daily_record_of_avg_clinical_pgs = 0.0;

    // int daily_record_of_num_r0_pgs = 0;
    // int daily_record_of_num_ra_pgs = 0;
    // int daily_record_of_num_rb_pgs = 0;
    // int daily_record_of_num_rab_pgs = 0;
    ...
    int report_daily_imported_cases = 0;
    ...
    void parasite_mutation(
        common::GenotypeManager* const gt_mgr,
        common::DrugManager* const dg_mgr,
        int current_time_step
    );
    void parasite_mutation_tact(
        common::GenotypeManager* const gt_mgr,
        common::DrugManager* const dg_mgr
        // int current_time_step
    );
    ...
    static void parasite_dominance_tact_kernel(
...
private:
    ...
    int find_rand_clean_system() const;

    inline bool is_infected(int system_index) const {
        return (this->head_pg_of[system_index] >= 0);
    }
    inline int head_pg_idx(int system_index) const {
        return system_index*kNum_pgs_per_system + this->head_pg_of[system_index];
    }
    inline int tail_pg_idx(int system_index) const {
        return system_index*kNum_pgs_per_system + this->tail_pg_of[system_index];
    }
    inline int pg_count(int system_index) const {
        return this->tail_pg_of[system_index] - this->head_pg_of[system_index] + 1;
    }
    inline void set_pg_type(int pg_index, common::ParasiteType pt) {
        this->pg_type_of[pg_index] = pt;
    }
